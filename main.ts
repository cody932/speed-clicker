info.onScore(100000, function () {
    game.gameOver(true)
})
controller.up.onEvent(ControllerButtonEvent.Pressed, function () {
    info.changeScoreBy(10)
})
controller.B.onEvent(ControllerButtonEvent.Pressed, function () {
    info.changeScoreBy(5)
})
info.onScore(1000, function () {
    mySprite.sayText("Upgrade unlocked you can start pressing S on the keyboard.")
})
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    info.changeScoreBy(1)
})
controller.left.onEvent(ControllerButtonEvent.Pressed, function () {
    info.changeScoreBy(100)
})
info.onCountdownEnd(function () {
    info.changeLifeBy(-1)
    info.startCountdown(20)
})
info.onScore(100, function () {
    mySprite.sayText("upgrade unlocked start pressing enter button on the key board")
})
info.onScore(10000, function () {
    mySprite.sayText("Upgrade unlocked you can start pressing D on the keyboard.")
})
controller.right.onEvent(ControllerButtonEvent.Pressed, function () {
    info.changeScoreBy(500)
})
info.onScore(2500, function () {
    mySprite.sayText("Upgrade unlocked you can start pressing A on the keyboard.")
})
controller.down.onEvent(ControllerButtonEvent.Pressed, function () {
    info.changeScoreBy(50)
})
info.onLifeZero(function () {
    game.gameOver(false)
})
info.onScore(500, function () {
    mySprite.sayText("Upgrade unlocked you can start pressing W on the keyboard.")
})
let mySprite: Sprite = null
scene.setBackgroundImage(img`
    
    `)
mySprite = sprites.create(img`
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . . 
    . . . . . . . . . . . . . . . f 
    `, SpriteKind.Player)
mySprite.setPosition(86, 119)
info.setScore(0)
info.setLife(3)
info.startCountdown(60)
info.stopCountdown()
mySprite.sayText("Save the world get to 100000 score start by pressing Z on the keyboard to gain score I will tell you when to press other buttons don't run out of lives first get to 100 score.")
pause(20000)
